...
CREATE TABLE `user2group` (
  ...
  CONSTRAINT `user2group_ibfk_2` FOREIGN KEY (`user_group_id`) REFERENCES `user_group` (`user_group_id`) ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
INSERT INTO `user_group` (`user_group_id`, `name`, `description`) VALUES
